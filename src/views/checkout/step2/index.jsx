...
const FormSchema = Yup.object().shape({
  ...
  mobile: Yup.object()
    .shape({
      ...
      value: Yup.string().required('Mobile number is required').matches(/^\d{11}$/, 'Mobile number must be at least 10 digits'),
    })
...
const ShippingDetails = ({ profile, shipping, subtotal }) => {
  useDocumentTitle('Check Out Step 2 | MacraCraft');
  useScrollTop();
  ...
  const initFormikValues = {
    ...
    isInternational: shipping.isInternational || true,
          ...
          >
            {() => (
              <Form>
                ...
                <br />
                {/*  ---- TOTAL --------- */}
                <ShippingTotal subtotal={Number(subtotal + 30000)} />
                <br />
                {/*  ----- NEXT/PREV BUTTONS --------- */}
